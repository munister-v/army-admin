...
async function tryAutoLogin() {
  ...
  try {
    ...
    if (!['admin','platform_admin'].includes(user?.role)) {
      showToast('Недостатньо прав для доступу.', 'error');
      api.setToken('');
      showAuth();
      return;
    }
...
async function loadDashboard() {
  try {
    ...
    rolesGrid.innerHTML = (d.by_role || []).map(r =>
      `<div class="role-badge">${roleNames[r.role] || r.role} <b>${r.cnt}</b></div>`
    ).join('');

    const tbody = document.getElementById('recentTxBody');
    tbody.innerHTML = (d.recent_tx || []).map(tx => `
      <tr>
        <td>${fmt(tx.created_at)}</td>
        <td><code style="font-size:.8rem">${escHtml(tx.account_number)}</code></td>
        ...
        <td style="color:var(--text-muted);max-width:220px;overflow:hidden;text-overflow:ellipsis">${escHtml(tx.description || '—')}</td>
      </tr>
    `).join('');
  } catch (err) {
  ...
  try {
    ...
    tbody.innerHTML = users.map(u => `
      <tr>
        ...
        <td><b>${escHtml(u.full_name)}</b></td>
        <td>${escHtml(u.phone || '—')}</td>
        <td style="color:var(--text-muted)">${escHtml(u.email || '—')}</td>
        <td>${roleBadge(u.role)}</td>
        <td style="color:var(--text-muted);font-size:.78rem">${escHtml(u.military_status || '—')}</td>
...
async function openUserModal(userId) {
  ...
  try {
    ...
    document.getElementById('roleChangeMsg').classList.add('hidden');
    document.getElementById('roleChangeMsg').className = 'form-msg hidden';
    document.getElementById('payoutMsg').classList.add('hidden');
    document.getElementById('payoutMsg').className = 'form-msg hidden';
...
document.getElementById('mPayoutBtn').addEventListener('click', async () => {
  ...
  try {
    ...
    msgEl.textContent = `Нараховано ${fmtMoney(amount)}. Новий баланс: ${fmtMoney(res.data.new_balance)}`;
    msgEl.className = 'form-msg success';
...
async function loadTransactions(offset = 0) {
  ...
  try {
    ...
    tbody.innerHTML = rows.map(tx => `
      <tr>
        <td style="color:var(--text-muted);font-size:.8rem">${fmt(tx.created_at)}</td>
        <td><span style="font-size:.82rem">${escHtml(tx.full_name || '—')}</span></td>
        <td><code style="font-size:.78rem">${escHtml(tx.account_number || '—')}</code></td>
        <td>${txTypeBadge(tx.tx_type)}</td>
        <td>${badge(tx.direction === 'in' ? 'in' : 'out', tx.direction === 'in' ? '↑ Зарах.' : '↓ Список.')}</td>
        <td class="${tx.direction === 'in' ? 'amount-in' : 'amount-out'}">${fmtMoney(tx.amount)}</td>
        <td style="color:var(--text-muted);max-width:200px;overflow:hidden;text-overflow:ellipsis;font-size:.82rem">${escHtml(tx.description || '—')}</td>
      </tr>
    `).join('');
...
function renderPagination(total, offset, limit) {
  ...
  let html = '';
  html += `<button class="page-btn" ${cur === 0 ? 'disabled' : ''} onclick="loadTransactions(${(cur-1)*limit})">‹ Попередня</button>`;
  const start = Math.max(0, cur - 2);
  const end = Math.min(pages - 1, cur + 2);
  if (start > 0) html += `<button class="page-btn" onclick="loadTransactions(0)">1</button>${start > 1 ? '<span style="color:var(--text-muted);padding:0 4px">…</span>' : ''}`;
  for (let i = start; i <= end; i++) {
    html += `<button class="page-btn ${i === cur ? 'active' : ''}" onclick="loadTransactions(${i*limit})">${i+1}</button>`;
  }
  if (end < pages - 1) html += `${end < pages - 2 ? '<span style="color:var(--text-muted);padding:0 4px">…</span>' : ''}<button class="page-btn" onclick="loadTransactions(${(pages-1)*limit})">${pages}</button>`;
  html += `<button class="page-btn" ${cur >= pages-1 ? 'disabled' : ''} onclick="loadTransactions(${(cur+1)*limit})">Наступна ›</button>`;
  bar.innerHTML = html;
}
...
/* ══════════════════════════════════════════════
   PAYOUTS PAGE
══════════════════════════════════════════════ */
let payoutSearchTimer = null;

document.getElementById('payoutUserSearch').addEventListener('input', e => {
  ...
  payoutSearchTimer = setTimeout(() => searchPayoutUsers(val), 350);
});

async function searchPayoutUsers(q) {
  try {
...
function selectPayoutUser(id, name, sub) {
  ...
  document.getElementById('selectedUserName').textContent = `${name} — ${sub}`;
...
async function loadAuditLogs() {
  try {
    ...
    const eventLabels = {
      ...
      transfer_out: 'Переказ (відправник)', transfer_in: 'Переказ (отримувач)',
